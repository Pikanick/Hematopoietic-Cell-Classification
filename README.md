...
### Known issue: missing model/weight files
`Models/Final-Model.hdf5`, `Weights/MV3_Weights.hdf5`, and `Weights/VGG16_Weights.hdf5`
are tracked with [Git LFS](https://git-lfs.com), but the actual binary objects were
never pushed to this repository's LFS storage (`git lfs pull` returns
"Object does not exist on the server" for all three) -- only the small LFS
pointer text files made it in. Until the real `.hdf5` files are pushed (with
`git lfs install` and `git lfs push --all origin main` from wherever the
trained weights still exist), `Model_Predict.py` and `Model_Test.py` will
fail to load them. This repo now has a `.gitattributes` marking `*.hdf5` as
LFS-tracked, which was also missing before -- without it, cloning this repo
would silently check out the pointer text files as if they were the actual
weights, instead of failing clearly like it does now.
